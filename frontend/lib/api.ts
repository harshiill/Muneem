...
export const chatApi = {
  ...
  sendMessageStream: async (message: string, onChunk: (text: string) => void, refreshContext: boolean = false) => {
    try {
      ...
      if (error instanceof TypeError && error.message.includes('timeout')) {
        throw new Error('Request timeout - chat service is taking too long to respond')
      }
      throw error
    }
  },
...
export const expenseApi = {
  ...
  markSplitSettled: async (splitId: number) => {
    const response = await apiClient.patch(`/expenses/splits/${splitId}/settle`)
    return response.data
  },
  ...
  updateDueStatus: async (dueId: number, status: 'pending' | 'paid' | 'overdue') => {
    const response = await apiClient.patch(`/expenses/dues/${dueId}/status?status=${status}`)
    return response.data
  },
}
